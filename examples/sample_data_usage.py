...
def main():
    ...
    print("=" * 50)
    specific_column_lineage = get_lineages(
        id=example_id, label_type="gold", columns=first_column
    )
    ...
    try:
        ...
        for i, (gold_item, generated_item) in enumerate(
            zip(gold_ordered, generated_ordered)
        ):
            gold_schema, gold_data = gold_item
            generated_schema, generated_data = generated_item
            print(
                f"  {i+1}. {gold_schema} == {generated_schema}: {gold_schema == generated_schema}"
            )
        ...
        if gold_ordered and generated_ordered:
            ...
            print(
                f"Performing batch evaluation on {len(gold_ordered)} schema columns..."
            )
            ...
            print("-" * 70)
            print(
                f"{'Schema Name':<25} {'Overall':<8} {'SrcSchema':<10} {'SrcTable':<8} {'Transform':<9} {'Aggreg':<8}"
            )
            print("-" * 70)
            ...
            for i, (gold_item, generated_item) in enumerate(
                ...
                assert (
                    gold_schema == generated_schema
                ), f"Schema mismatch: {gold_schema} != {generated_schema}"

                # Evaluate this schema
                results = evaluator.evaluate(generated_data, gold_data)

                # Store results
                batch_results.append({"schema": gold_schema, "results": results})
            ...
            avg_scores = {
                metric: total / num_schemas for metric, total in total_scores.items()
            }
            ...
            best_schema = max(
                batch_results, key=lambda x: x["results"].get("overall", 0)
            )
            worst_schema = min(
                batch_results, key=lambda x: x["results"].get("overall", 0)
            )
            ...
            for category, (min_score, max_score) in score_ranges.items():
                ...
                print(
                    f"  {category.capitalize()}: {count}/{num_schemas} ({percentage:.1f}%)"
                )
        ...
        print(
            "This likely means the generated data doesn't have corresponding lineages for this dataset."
        )
