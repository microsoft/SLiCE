...
def main():
    ...
    print(f"Default Overall Score: {default_results.get('overall', default_results.get('all', 0)):.4f}")
    ...
    metadata_results = metadata_evaluator.evaluate(
        ...
        ground_truth_with_metadata
    )
    
    print(f"Metadata-enabled Overall Score: {metadata_results.get('overall', metadata_results.get('all', 0)):.4f}")
