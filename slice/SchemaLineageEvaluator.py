...
from slice.constants import EvaluationDefaults, LineageComponents, ScoreComponents
...
class SchemaLineageEvaluator:
    """
    Evaluates schema lineage extraction by comparing model predictions with gold
    standards. Supports component-wise evaluation and weighted averaging for final
    scores.
    """
    ...
    def __init__(
        ...
        self.source_table_weights = (
            source_table_weights or EvaluationDefaults.DEFAULT_SOURCE_TABLE_WEIGHTS
        )
        self.transformation_weights = (
            transformation_weights or EvaluationDefaults.DEFAULT_TEXT_SIMILARITY_WEIGHTS
        )
        self.aggregation_weights = (
            aggregation_weights or EvaluationDefaults.DEFAULT_TEXT_SIMILARITY_WEIGHTS
        )
        self.metadata_weights = (
            metadata_weights or EvaluationDefaults.DEFAULT_TEXT_SIMILARITY_WEIGHTS
        )
        ...
        self.weighted_bleu_calculator = WeightedBleuCalculator(
            ...
            special_operators=self.operators["sql"]
            | self.operators["python"]
            | self.operators["csharp"],
        )
    ...
    def _validate_lineage(self, lineage: Dict[str, str]) -> bool:
        ...
        if self.has_metadata:
            required_attrs = [
                LineageComponents.SOURCE_SCHEMA,
                LineageComponents.SOURCE_TABLE,
                LineageComponents.TRANSFORMATION,
                LineageComponents.AGGREGATION,
        ...
        for attr in required_attrs:
            ...
            if not isinstance(lineage[attr], str):
                logging.warning(
                    f"Attribute {attr} is not a string or is empty: {lineage[attr]}"
                )
                return False
        return True
    ...
    def _validate_parameters(self):
        ...
        if self.has_metadata:
            if len(self.weights) != 4:
                raise ValueError(
                    "Weights must have 4 components: source_tables, transformation, aggregation, metadata"
                )
        else:
            if len(self.weights) != 3:
                raise ValueError(
                    "Weights must have 3 components: source_tables, transformation, aggregation"
                )
        ...
        if abs(sum_source_table_weights - 1.0) > tolerance:
            raise ValueError(
                f"Source table weights must sum to 1.0, but got {sum_source_table_weights}"
            )

        sum_transformation_weights = sum(self.transformation_weights.values())
        if abs(sum_transformation_weights - 1.0) > tolerance:
            raise ValueError(
                f"Transformation weights must sum to 1.0, but got {sum_transformation_weights}"
            )

        sum_aggregation_weights = sum(self.aggregation_weights.values())
        if abs(sum_aggregation_weights - 1.0) > tolerance:
            raise ValueError(
                f"Aggregation weights must sum to 1.0, but got {sum_aggregation_weights}"
            )
        ...
        if self.has_metadata:
            ...
            if abs(sum_metadata_weights - 1.0) > tolerance:
                raise ValueError(
                    f"Metadata weights must sum to 1.0, but got {sum_metadata_weights}"
                )
    ...
    def evaluate(
        self, predicted_lineage: Dict[str, str], gold_lineage: Dict[str, str]
    ) -> Dict[str, float]:
        """
    ...
    def evaluate_batch(
        self,
        predicted_lineage: List[Dict[str, str]],
        gold_lineage: List[Dict[str, str]],
    ) -> List[Dict[str, float]]:
        """
        ...
        if len(predicted_lineage) != len(gold_lineage):
            raise ValueError(
                f"Number of predicted lineages ({len(predicted_lineage)}) "
        ...
        if num_processes <= 1 or len(predicted_lineage) <= 1:
            ...
            return [
                self.evaluate(pred, gold)
                for pred, gold in zip(predicted_lineage, gold_lineage)
            ]
        ...
        try:
            ...
            with mp.Pool(processes=num_processes) as pool:
                ...
                results = pool.starmap(
                    evaluate_func, zip(predicted_lineage, gold_lineage)
                )
    ...
    def evaluate_source_schema(
        self, predicted_lineage: SchemaLineage, gold_lineage: SchemaLineage
    ) -> float:
        """
    ...
    def evaluate_source_tables(
        ...
        weighted_score = (
            self.source_table_weights["f1"] * f1_score
            + self.source_table_weights["fuzzy"] * fuzzy_score
        )
    ...
    def _evaluate_text_similarity(
        ...
        predicted_text: str,
        gold_text: str,
        weights: Dict[str, float] = {"bleu": 0.5, "weighted_bleu": 0.3, "ast": 0.2},
    ) -> float:
        """
        ...
        bleu_score = calc_bleu_score(
            predicted_text_cleaned,
            gold_text_cleaned,
            special_operators=self.operators["sql"] | self.operators["python"],
        )

        # calculate the weighted bleu score
        weighted_bleu_score = self.weighted_bleu_calculator.calc_score(
            predicted_text_cleaned, gold_text_cleaned
        )

        # calculate the ast score
        ast_score = self.ast_calculator.calc_ast_score(
            predicted_text_cleaned, gold_text_cleaned
        )
    ...
    def evaluate_metadata(
        ...
        """
        return self._evaluate_text_similarity(
            predicted_lineage.metadata, gold_lineage.metadata, self.metadata_weights
        )
