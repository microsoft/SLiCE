...
Examples:
  ...
  slice-eval --weights source_table=0.5,transformation=0.3,aggregation=0.2 predicted.json ground_truth.json
    ...
    parser.add_argument(
        'predicted', 
        help='Path to JSON file containing predicted lineage'
    )
    parser.add_argument(
        'ground_truth', 
        help='Path to JSON file containing ground truth lineage'
    )
    parser.add_argument(
        '--metadata', 
        action='store_true',
    ...
    if args.weights:
        try:
            ...
            print("Expected format: source_table=0.4,transformation=0.4,aggregation=0.2")
            sys.exit(1)
    
    # Initialize evaluator
    evaluator = SchemaLineageEvaluator(
        weights=weights,
        has_metadata=args.metadata
    )
