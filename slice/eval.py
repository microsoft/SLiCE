...
def calc_fuzzy_match_score(str1: str, str2: str) -> float:
    """
...
def calc_precision_score(predicted: Set[str], truth: Set[str]) -> float:
    """
...
def calc_recall_score(predicted: Set[str], truth: Set[str]) -> float:
    """
    ...
    if len(truth) == 0 and len(predicted) != 0:
        return 0
    tp = len(predicted.intersection(truth))
    recall = float(tp / len(truth))

    return recall


def calc_f1_score(predicted: Set[str], truth: Set[str]) -> float:
    """
...
def calc_bleu_score(
    ...
    try:
        score = sentence_bleu(
            references, predicted_tokens, weights=weights, smoothing_function=smoothing
        )
        return score
...
class WeightedBleuCalculator:
    ...
    def _brevity_penalty(self, truth_len: int, predicted_len: int) -> float:
        """
    ...
    def calc_score(self, predicted: str, truth: str, weights: Tuple = None) -> float:
        ...
        # Tokenize the text using the specialized code tokenizer
        predicted_tokens = _code_tokenize(
            predicted, special_operators=self.special_operators
        )
        ...
        if len(token_weights) == 0:
            return calc_bleu_score(
                predicted, truth, weights, special_operators=self.special_operators
            )
        ...
        for i, weight in enumerate(weights, start=1):
            numerator, denominator = self._modified_precision(
                truth_tokens, predicted_tokens, token_weights, i
            )
        ...
        for i, ((numerator, denominator), weight) in enumerate(
            zip(precisions, weights)
        ):
            if numerator > 0:
...
class ASTCalculator:
    ...
    def _get_parser(self, lang: str):
        ...
        if lang not in self._parsers:
            try:
                ...
                raise ValueError(
                    f"Failed to get parser for language '{lang}': {str(e)}"
                )
    ...
    def calc_language_confident(self, code: str) -> Tuple[float, float, float]:
        ...
        python_matches = sum(
            code_lower.count(feature.lower()) for feature in self.python_syntax
        )
        sql_matches = sum(
            code_lower.count(feature.lower()) for feature in self.sql_syntax
        )
        csharp_matches = sum(
            code_lower.count(feature.lower()) for feature in self.csharp_syntax
        )
        total_matches = python_matches + sql_matches + csharp_matches
        if total_matches == 0:
            return 0.0, 0.0, 0.0
        return (
            min(1, python_matches / total_matches),
    ...
    def calc_ast_similarity(self, predicted: str, truth: str, lang: str) -> float:
        """
        ...
        match_count = sum(
            1 for sub_tree, _ in predicted_sub_tree if sub_tree in truth_sexps
        )
    ...
    def calc_ast_score(self, predicted: str, truth: str) -> float:
        """
        ...
        """
        python_confident, sql_confident, csharp_confident = (
            self.calc_language_confident(predicted)
        )
        ...
        return (
            python_confident * python_score
            + sql_confident * sql_score
            + csharp_confident * csharp_score
        )
