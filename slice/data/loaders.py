...
INVENTORY_COLUMNS = ['id', 'column_count', 'difficulty', 'last_modified_date', 'language']
...
def get_pipeline_script(id:str="80963c21-378a-4d8d-e948-feeb9d74c049") -> str:
    ...
    if id in data:
        return data[id].get('pipeline_script', '')
    
    raise ValueError(f"Dataset ID '{id}' not found in gold data")

def get_lineages(id:str="80963c21-378a-4d8d-e948-feeb9d74c049", label_type: str = 'gold', columns: Optional[str] = None) -> Dict[str, Any]:
    ...
    if label_type == 'gold':
        ...
        raise ValueError(f"Invalid label_type '{label_type}'. Must be 'gold' or 'generated'")
    ...
    if id in data and lineage_key in data[id]:
        ...
        if columns:
            if columns in lineage_data:
                ...
                raise ValueError(f"Column '{columns}' not found in lineage for dataset '{id}'")
        else:
