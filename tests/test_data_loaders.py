...
class TestDataExistence:
    ...
    def test_gold_json_exists(self):
        ...
        for dataset_id, dataset_info in data.items():
            if isinstance(dataset_info, dict):
                assert (
                    "lineage" in dataset_info
                ), f"Dataset {dataset_id} missing 'lineage' key"
                assert (
                    "pipeline_script" in dataset_info
                ), f"Dataset {dataset_id} missing 'pipeline_script' key"
                break
    ...
    def test_id_match(self):
        ...
        generated_file = (
            Path(__file__).parent.parent / "slice" / "data" / "datasets" / GENERATED
        )
        ...
        assert (
            inventory_ids == gold_lineages
        ), "Inventory IDs do not match gold lineages"
        assert (
            inventory_ids == generated_lineages
        ), "Inventory IDs do not match generated lineages"
    ...
    def test_generated_json_exists(self):
        ...
        for dataset_id, dataset_info in data.items():
            if isinstance(dataset_info, dict):
                assert (
                    "parsed_lineages" in dataset_info
                ), f"Dataset {dataset_id} missing 'parsed_lineages' key"
...
class TestListInventory:
    ...
    def test_list_inventory_returns_dataframe(self):
        ...
        assert isinstance(
            result, pd.DataFrame
        ), "list_inventory should return a DataFrame"
...
class TestGetLineages:
    ...
    def test_get_lineages_gold_type(self):
        ...
        for column_name, lineage_info in lineages.items():
            assert isinstance(
                lineage_info, dict
            ), f"Lineage info for {column_name} should be dict"
            expected_keys = [
                "source_schema",
                "source_table",
                "transformation",
            ...
            for key in expected_keys:
                assert (
                    key in lineage_info
                ), f"Missing key '{key}' in lineage for {column_name}"
...
class TestPrepareBatchEvaluation:
    ...
    def test_prepare_batch_evaluation_matching_schemas(self):
        ...
        gold_ordered, generated_ordered = prepare_batch_evaluation(
            gold_data, generated_data
        )
    ...
    def test_prepare_batch_evaluation_extra_in_generated(self):
        ...
        gold_data = {"column1": {"source_schema": "col1"}}
        generated_data = {
            "column1": {"source_schema": "col1_gen"},
...
class TestListPipelineScripts:
    ...
    def test_list_pipeline_scripts_only_gold_type(self):
        ...
        if len(scripts) > 0:
            ...
            assert label_types == {
                "gold"
            }, "Should only have gold label type (pipeline scripts only in gold data)"
...
class TestIntegration:
    ...
    def test_real_data_pipeline(self):
        ...
        try:
            ...
            gold_ordered, generated_ordered = prepare_batch_evaluation(
                gold_lineages, generated_lineages
            )
            assert len(gold_ordered) == len(
                generated_ordered
    ...
    def test_data_consistency(self):
        ...
        assert (
            len(missing_in_gold) == 0
        ), f"IDs in inventory but not in gold data: {missing_in_gold}"
