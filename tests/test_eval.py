...
def test_code_tokenize():
    ...
    assert _code_tokenize("if(x>0){return x;}") == ["if", "(", "x", ">", "0", ")", "{", "return", "x", ";", "}"]
    ...
    assert _code_tokenize("x -= 1") == ["x", "-=", "1"]
    assert _code_tokenize("if x >= 0 and y <= 10:") == ["if", "x", ">=", "0", "and", "y", "<=", "10", ":"]
    ...
    assert _code_tokenize("customer->>'name'") == ["customer", "->", ">", "'", "name", "'"]
    ...
    assert _code_tokenize("x !== y", special_operators=custom_ops) == ["x", "!==", "y"]
    assert _code_tokenize("result = condition ?: true_val", special_operators=custom_ops) == [
...
def test_calc_bleu_score():
    ...
    assert 0.3 < score6 < 0.7  # Should be moderate due to similar structure but different operations 
...
def test_weighted_bleu_calculator():
    ...
    assert score5 > score6 # add lower weight to non-keywords, more penalty for not matching keywords
...
def test_weighted_bleu_calculator_with_special_operators():
    ...
    assert 0.7 < score3 < 1.0  # Should be high despite the different comparison operator
...
def test_ast_calculator_init():
    ...
    assert calculator.language_map == {'python': 'python', 'sql': 'sql', 'csharp': 'c_sharp'}
...
def test_ast_calculator_edge_cases():
    ...
    assert calculator.calc_ast_similarity(formatted_code1, formatted_code2, "python") == 1.0
...
def test_ast_calculator_csharp_edge_cases():
    ...
    bad_code = "public class User { public string Name { get; set; }"  # Missing closing brace
    ...
    assert calculator.calc_ast_similarity(formatted_code1, formatted_code2, "csharp") == 1.0
