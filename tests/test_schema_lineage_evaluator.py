...
def test_validate_parameters():
    # Test valid weights
    valid_weights = {LineageComponents.SOURCE_TABLE: 0.4, LineageComponents.TRANSFORMATION: 0.4, LineageComponents.AGGREGATION: 0.2}
    ...
    with pytest.raises(ValueError):
        SchemaLineageEvaluator(weights={
    ...
    evaluator = SchemaLineageEvaluator(weights=valid_weights_with_metadata, has_metadata=True)
...
def test_validate_lineage():
    ...
    assert evaluator_with_metadata._validate_lineage(valid_lineage_with_metadata) is True
    ...
    assert evaluator_with_metadata._validate_lineage(invalid_lineage_with_metadata) is False
...
def test_evaluate_source_schema():
    ...
    for test_case in test_cases:
        score = evaluator.evaluate_source_schema(test_case['predicted'], test_case['gold'])
        
        if 'expected_score' in test_case:
            assert score == test_case['expected_score'], f"Expected score {test_case['expected_score']} but got {score}"
        else:
            assert test_case['expected_min'] <= score <= test_case['expected_max'], \
...
"IDEAsAvailableUnits","IDEAsCloudAscentData","IDEAsDomains","IDEAsFirstPaidDates","IDEAsInternal","IDEAsMSSales","IDEAsPublicSector","IDEAsSKU","IDEAsSubscription","IDEAsTenantTags","IDEAsViral","IDEAsFastTrackTenants","IDEAsCALC","IDEAsHasWorkloads"})""",
    ...
    for test_case in test_cases:
        score = evaluator.evaluate_source_tables(test_case['predicted'], test_case['gold'])
...
def test_evaluate_transformation():
    ...
    for test_case in test_cases:
        score = evaluator.evaluate_transformation(test_case['predicted'], test_case['gold'])
        
        if 'expected_score' in test_case:
            assert score == test_case['expected_score'], f"ID: {test_case['id']} - Expected score {test_case['expected_score']} but got {score}"
        else:
            assert test_case['expected_min'] <= score <= test_case['expected_max'], \
...
def test_evaluate_aggregation():
    ...
    for test_case in test_cases:
        score = evaluator.evaluate_aggregation(test_case['predicted'], test_case['gold'])
...
def test_evaluate_metadata(evaluator_with_metadata):
    ...
    for test_case in test_cases:
        score = evaluator_with_metadata.evaluate_metadata(test_case['predicted'], test_case['gold'])
...
def test_evaluate_wrong_schema_name():
    ...
    with pytest.raises(ValueError):
        evaluator_with_metadata.evaluate(predicted_without_metadata, gold_without_metadata)
...
def test_evaluate():
    ...
    expected_overall = (
        evaluator.weights[LineageComponents.SOURCE_TABLE] * result[LineageComponents.SOURCE_TABLE] +
        evaluator.weights[LineageComponents.TRANSFORMATION] * result[LineageComponents.TRANSFORMATION] +
        evaluator.weights[LineageComponents.AGGREGATION] * result[LineageComponents.AGGREGATION]
    )
    ...
    result_with_metadata = evaluator_with_metadata.evaluate(predicted_with_metadata, gold_with_metadata)
    ...
    expected_overall_with_metadata = (
        evaluator_with_metadata.weights[LineageComponents.SOURCE_TABLE] * result_with_metadata[LineageComponents.SOURCE_TABLE] +
        evaluator_with_metadata.weights[LineageComponents.TRANSFORMATION] * result_with_metadata[LineageComponents.TRANSFORMATION] +
        evaluator_with_metadata.weights[LineageComponents.AGGREGATION] * result_with_metadata[LineageComponents.AGGREGATION] +
        evaluator_with_metadata.weights[LineageComponents.METADATA] * result_with_metadata[LineageComponents.METADATA]
    )
...
def test_normalize_quotes():
    ...
    assert normalize_quotes('SELECT "column" FROM table') == "SELECT 'column' FROM table"
    assert normalize_quotes("SELECT 'column' FROM table") == "SELECT 'column' FROM table"
    
    # Test multiple quotes
    assert normalize_quotes('SELECT "col1", "col2" FROM "table"') == "SELECT 'col1', 'col2' FROM 'table'"
